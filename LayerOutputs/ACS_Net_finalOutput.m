function output = ACS_Net_finalOutput(image)
tic
...
ImEntropy = roundn(ImEntropy09,-3);%保留5位小数；
%获取熵数组，并排位


global EArray;%全局变量：获取主函数里的熵数组；
index = find(EArray == ImEntropy);
fprintf("index为：%d\n",index);
fprintf("小块的熵为：%d\n",ImEntropy);
%选择是否使用GPU！
...
%%根据图像的熵来switch采样率；
rank =(index+1)/length(EArray);%计算该块的熵排位
%rank = 0.2;
if rank < 0.2
    Rate = 0.01;   
elseif  rank <0.5
...
global mode;
if(mode == 2)%选择模式2：使用固定采样率；
    Rate = ConstRate;  %固定采样率
end
switch Rate
    case 0.01
        initRec =  s01Init;

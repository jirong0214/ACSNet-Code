function [inputs, labels, set] = patchesInit2Deep(size_input,size_label,stride,folder_inputs,folder_labels,mode,max_numPatches,batchSize)
...
for i = 1 : length(ext)
    filepaths_inputs = cat(1,filepaths_inputs, dir(fullfile(folder_inputs, ext{i})));
end

for i = 1 : length(filepaths_inputs)
    image = imread(fullfile(folder_inputs,filepaths_inputs(i).name)); % uint8
